...
const MD_SRC = [
  ...
  '| Costs | 2100 |',
  '',
  'Arrows → rupees ₹ and naïve café must survive as sanitized text.',
].join('\n');
...
check('md source detected', (await page.locator('#cdetected').innerText()).includes('MD'));
{
  ...
  // Regression: "WinAnsi cannot encode" used to throw here. → must become ->,
  // ₹ must degrade to ?, Latin-1 chars (ï, é) must survive untouched.
  check(
    'MD -> PDF sanitizes non-WinAnsi chars instead of failing',
    t.includes('Arrows -> rupees') && t.includes('naïve café') && !t.includes('→') && !t.includes('₹'),
    (t.match(/Arrows[^\n]*/) ?? ['missing']).join('').slice(0, 70),
  );
}
